...
6. The `partition_table.bin` file is generated by `gen_esp32part.py` tool comes with ESP-IDF, you can generate it with the following command if you have ESP-IDF installed:
    ```bash
    /path/to/esp-idf/components/partition_table/gen_esp32part.py partitions_single_app.csv partition_table.bin
    ```

7. The `esp32s3-bootloader.bin` is the official bootloader binary, you can find it in the [`espflash`](https://github.com/esp-rs/espflash) repo, usually under the `espflash/resources/bootloaders` directory.

8. Exit the download mode and reset the board, you should see the new USB HID device on your host.
